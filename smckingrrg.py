...
st.set_page_config(page_title="Professional Sector-Stock Matrix RRG", layout="wide")
...
st.markdown("""
    ...
    div[data-testid="stDataFrame"] { background-color: #1a2130 !important; }
    </style>
""", unsafe_allow_html=True)

st.title("🚀 Professional Sector & Stock Rotation Graph (RRG)")
...
# Flatten out tickers securely for the back-end pipeline downloader
all_unique_stocks = list(set([stock for sector in sector_map.values() for stock in sector]))
ticker_benchmark = "^CRSLDX" # Nifty 500 reference index
...
# --- 3. CRASH-PROOF STABLE DATA DOWNLOAD PIPELINE ---
with st.spinner("Downloading Global Live Market Feed Matrix..."):
    # Standard linear download format to strictly eliminate column level indexing mismatch issues
...
if not raw_data.empty and 'Close' in raw_data:
    ...
    # Compile dataset to a linear index object
    master_summary = []
    for stock in all_unique_stocks:
    ...
    # --- 4. STRUCTURAL GRID DISPATCH [1, 2.3] ---
    col_left, col_right = st.columns([1, 2.3])

    active_tickers = []

    with col_left:
        st.markdown("### 🗂️ Sector-Wise Folders")
        
        # Iterating vertical linear folder tree matching Strike Money premium UX
        for sector_name, stock_list in sector_map.items():
            ...
            # Clickable folder expanding block dropdown rows
            with st.expander(f"📁 {sector_name}"):
                edited_df = st.data_editor(
                    df_subset[["Active", "SYMBOL", "QUADRANT", "PERF %"]],
                    column_config={
                    ...
                    key=f"exp_{sector_name}"
                )
                
                # Stack up checkboxes into the chart plotter engine
                sub_active = edited_df[edited_df["Active"] == True]["SYMBOL"].tolist()
                active_tickers.extend([s + ".NS" for s in sub_active])

    with col_right:
        st.markdown("### 📊 High-Fidelity Rotation Canvas")
        ...
        if len(active_tickers) >= 1:
            ...
            # Auto Scale Bounds Calculation Engine
            ...
            # Execute Spline Interpolations curves
            ...
            st.info("Left folder rows ko open karke stocks par tick lagaiye, live matrix mathematical lines automatic load ho jayengi.")
else:
    st.error("Live streaming pipeline sync down.")
